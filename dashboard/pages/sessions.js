              ...
              {sessions.map((s, i) => {
                ...
                return (
                  <tr key={i} className={`border-b border-border ${s.paper_mode ? 'bg-paper/5' : ''}`}>
                    ...
                    <td className="py-2 px-2 text-right font-financial">{formatCurrency(s.start_wallet)}</td>
                    <td className="py-2 px-2 text-right font-financial">{formatCurrency(s.end_wallet)}</td>
                    <td className={`py-2 px-2 text-right font-financial ${pnl >= 0 ? 'text-profit' : 'text-loss'}`}>{pnl >= 0 ? '+' : ''}{formatCurrency(pnl)}</td>
                    <td className={`py-2 px-2 text-right font-financial ${pnl >= 0 ? 'text-profit' : 'text-loss'}`}>{pnlPct >= 0 ? '+' : ''}{pnlPct.toFixed(1)}%</td>
